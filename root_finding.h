...
double newton_raph( double (*function)(double), double x1, double x2, double xacc){
  ...
  if (xnew >= x1 && xnew <= x2){
    ...
    if (x1 < x2){
      return newton_raph( function, x1+0.01, x2, xacc);
    }
    else{
...
double false_pos( double(*function)(double), double xl, double xr, double yacc){
  int counter = 0;
  double x_new = xr;
  while (fabs(function(x_new)) >= yacc){     // iterate until function converges within accuracy
    ...
    x_new = xr - y2 * (xr - xl) / (y2 - y1);
    if (y1 * function(x_new) > 0){
      xl = x_new;
    }
    else{
      xr = x_new;
  ...
  printf("The number of iterations in False Position method were: %d\n", counter);
  return x_new;
}
